...
def model():
    ...
    pipeline = Pipeline(steps = [                                             #? Modelo de clasificador (SVM)
                        ...
                        ('classifier', MLPClassifier(max_iter = 15000, random_state=42))  #? Se ajusta el clasificador SVM con kernel RBF
                                ]
                        )
...
def optimize_hyperparameters(model, n_splits, x_train, y_train, scoring):     #? Paso 4: Optimización de hiperparámetros
    estimator = GridSearchCV(
        estimator = model,
        param_grid = {
            'pca__n_components': [None],                                      #? Número de componentes principales para PCA
            'selectkbest__k': [20],                                           #? Selección de las mejores 20 características
            'classifier__hidden_layer_sizes': [(50, 30, 40, 60)],             #? Tamaño de las capas ocultas en la red neuronal MLP (4 capas con 50, 30, 40, 60 neuronas respectivamente)
            'classifier__alpha': [0.28],                                      #? Parámetro de regularización (alpha) para controlar el sobreajuste
            'classifier__learning_rate_init': [0.001],                        #? Tasa de aprendizaje inicial para la optimización del MLP
        },
